...
import { LayoutDashboard, ArrowLeftRight, PieChart, WalletCards, LogOut } from 'lucide-react'
...
function App() {
  ...
  return <div className="shell"><aside><div className="brand"><span className="brand-mark">F</span><span>fluent</span></div><nav>{nav.map(({id,label,icon:Icon})=><button key={id} className={page===id?'active':''} onClick={()=>setPage(id)}><Icon size={19}/>{label}</button>)}</nav><div className="sidebar-bottom"><div className="user-email">{user.email}</div><button onClick={signOut}><LogOut size={18}/> Sign out</button></div></aside><main><Page goTo={setPage}/></main></div>
}
